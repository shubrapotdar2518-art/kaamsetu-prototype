...
export const ChooseRolePage: React.FC = () => {
  ...
  const { t } = useLanguage();
  const { chooseRoleAndSave, showToast } = useApp();
  ...
  const handleSelectWorker = async () => {
    ...
    setIsSaving(true);
    try {
      await chooseRoleAndSave("worker");
      navigate("/worker-details");
    } catch (error) {
      showToast(
        error instanceof Error
          ? error.message
          : "Please sign up with email first.",
        "error",
      );
    } finally {
      setIsSaving(false);
  ...
  const handleSelectEmployer = async () => {
    ...
    setIsSaving(true);
    try {
      await chooseRoleAndSave("employer");
      ...
      <main className="max-w-3xl w-full mx-auto my-auto py-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div
            ...
            className={`group relative bg-white hover:bg-gradient-to-b hover:from-white hover:to-[#ECFDF5] rounded-3xl p-6 sm:p-8 border-2 border-gray-200/90 hover:border-[#15803D] shadow-sm hover:shadow-xl transition-all flex flex-col justify-between ${
          ...
          >
            <div>
              ...
              <div className="text-center mb-4">
                ...
                <p className="text-xs sm:text-sm text-gray-600 font-medium mt-1">
                  {t(
                    "workerTagline",
                    "Find daily wage jobs & work opportunities",
                  )}
                </p>
              </div>
              ...
              <div className="space-y-2 mb-6 text-xs text-gray-600">
                <div className="flex items-center gap-2">
                  ...
                  <span>
                    {t(
                      "workerBenefit1",
                      "Instant job alerts in your local area",
                    )}
                  </span>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  <span>
                    {t(
                      "workerBenefit2",
                      "Direct wage deposit & government welfare access",
                    )}
                  </span>
                </div>
              </div>
            ...
            >
              <span>{isSaving ? "Saving..." : "Continue as Worker"}</span>
          ...
          <div
            ...
            className={`group relative bg-white hover:bg-gradient-to-b hover:from-white hover:to-[#FFF7ED] rounded-3xl p-6 sm:p-8 border-2 border-gray-200/90 hover:border-[#F97316] shadow-sm hover:shadow-xl transition-all flex flex-col justify-between ${
              isSaving ? "opacity-60 cursor-not-allowed" : "cursor-pointer"
            }`}
          >
            <div>
              ...
              <div className="space-y-2 mb-6 text-xs text-gray-600">
                <div className="flex items-center gap-2">
                  ...
                  <span>
                    {t(
                      "employerBenefit1",
                      "Verified skilled & unskilled workers nearby",
                    )}
                  </span>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  <span>
                    {t(
                      "employerBenefit2",
                      "Post emergency job requirements in 60 seconds",
                    )}
                  </span>
                </div>
              </div>
            ...
            >
              <span>{isSaving ? "Saving..." : "Continue as Employer"}</span>
              <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
            </button>
          </div>
        </div>
