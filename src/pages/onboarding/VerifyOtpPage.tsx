...
export const VerifyOtpPage: React.FC = () => {
  ...
  const handleResend = () => {
    ...
    setTimeout(() => {
      ...
      if (isEmailMode) {
        showToast(
          `New 6-digit verification code sent to ${userProfile.email || "your email"}!`,
        );
      } else {
        showToast(
          `New 6-digit OTP sent to +91 ${userProfile.phone || "your mobile"}!`,
        );
      }
    }, 600);
  };
  ...
  return (
    <div className="min-h-screen bg-[#F8FAF8] flex flex-col justify-between p-4 sm:p-6 lg:p-8">
          ...
          <button
            ...
            onClick={() =>
              navigate("/create-account", {
                state: { method: isEmailMode ? "email" : "mobile" },
              })
            }
      ...
      <footer className="text-center text-xs text-gray-400 py-2">
        {isEmailMode
          ? "Email code verification • KaamSetu"
          : "Instant SMS OTP verification • KaamSetu"}
      </footer>
    </div>
  );
};
