...
import { auth } from '../../../kaamsetu-prototype/backend/firebase-config';
  ...
  const handleKeyDown = (index: number, e: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (auth.currentUser?.emailVerified) {
        ...
        setVerificationError('Not verified yet. Please check your inbox and click the link first.');
      }
    } catch (error) {
    ...
    if (isEmailMode) {
      try {
        ...
        setCountdown(30);
        showToast(`Verification email resent to ${userProfile.email || 'your email'}!`);
      } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-[#F8FAF8] flex flex-col justify-between p-4 sm:p-6 lg:p-8">
      ...
      <main className="max-w-xl w-full mx-auto my-auto py-6">
        <div className="bg-white rounded-3xl p-6 sm:p-8 border border-gray-100 shadow-sm text-center">
          ...
          <p className="text-xs sm:text-sm text-gray-500 mt-1 max-w-sm mx-auto leading-relaxed">
            {isEmailMode ? 'A verification link has been sent to' : 'Enter the 6-digit OTP sent to'}{' '}
            ...
            {isEmailMode ? '. Please verify to continue.' : ''}
          </p>
            ...
            <div className="mt-8 space-y-6">
              ...
              <div className="bg-gray-50 border border-gray-200 rounded-2xl p-4 text-sm text-gray-700">
                               We've sent a verification link to your email. Click the link in that email — this page will automatically continue once you're verified.
              </div>
      ...
      <footer className="text-center text-xs text-gray-400 py-2">
               {isEmailMode ? 'Email link verification • KaamSetu' : 'Instant SMS OTP verification • KaamSetu'}
      </footer>
    </div>
  );
};
